      ...
      {data.map((val, i) => (
        ...
        >
          <div
            style={{
              width: "100%",
              height: Math.round((val / max) * height * 0.85),
              background:
    ...
    <div>
      <div className="section-header">
        ...
        <p className="section-subtitle">
          Your Bayan Space activity and growth at a glance
        </p>
      </div>
      ...
      <div className="settings-card">
        ...
        <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
          {TOP_STATS.map((s) => (
            <div className="stat-card" key={s.label} style={{ padding: 12, border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
              <div className="stat-value" style={{ color: s.color, fontSize: 20, fontWeight: 700 }}>
                {s.value}
              </div>
              <div className="stat-label" style={{ fontSize: 12, color: "var(--text3)" }}>{s.label}</div>
              <div className="stat-change" style={{ fontSize: 11 }}>{s.change}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="chart-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div className="chart-card" style={{ padding: 16, border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
          <div className="chart-card-title" style={{ fontWeight: 600 }}>Weekly Views</div>
          <div className="chart-card-sub" style={{ fontSize: 12, color: "var(--text3)" }}>Last 7 days</div>
          <BarChart data={WEEKLY} labels={DAYS} color="var(--gold)" height={80} />
        </div>
        <div className="chart-card" style={{ padding: 16, border: "1px solid var(--border)", borderRadius: "var(--radius-sm)" }}>
          <div className="chart-card-title" style={{ fontWeight: 600 }}>Monthly Growth</div>
          <div className="chart-card-sub" style={{ fontSize: 12, color: "var(--text3)" }}>Views this year</div>
      ...
      <div className="settings-card" style={{ marginTop: 12 }}>
        <div className="card-title">
          <span className="card-title-icon">◈</span> Reading Activity
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          {[
            ...
            <div
              key={item.label}
              style={{ textAlign: "center", flex: 1, direction: "ltr" }}
            >
...
export default React.memo(StatisticsSettings);
